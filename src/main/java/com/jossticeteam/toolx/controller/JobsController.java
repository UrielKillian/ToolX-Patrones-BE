...
public class JobsController {
    ...
    public Page<JobResource> getAllJobsByUserId(@PathVariable Long userId, Pageable pageable){
        ...
        List<JobResource>resources = jobPage.getContent().stream().map(
    ...
    public JobResource createJob(@PathVariable Long userId,
    ...
    public JobResource updateJob(@PathVariable Long userId,
    ...
    private Job convertToEntity(SaveJobResource resource)   {return mapper.map(resource,Job.class);}
    private JobResource convertToResource(Job entity){return mapper.map(entity, JobResource.class);}
